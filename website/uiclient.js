...
const Button = ({ background, icon, className, onClick, cycle, selected, value =  "Sign Up "}) => (
  <button
    className={className}
    css={{
      fontFamily,
...
const UIClient = ({ config, handleUserAccept, handleUserReject, newReward, step, cycle, selectedOption, rerenderUIClient }) => {
  const showVision = () => {
    return config.buttonPosition ===  "vision" ? true : false;
  };
  const showHero = () => {
    return config.buttonPosition === "hero" ? true : false;
  };
  useEffect(() => {    
    console.log("[Website - UI Client]step", step)
    if ( step != 10 ) return;    
    const [accept, reject, ...rest] = document.getElementsByTagName('button');
    console.log("accept, reject", accept, reject);
    if(newReward == 1){
      accept.click();
    }else{
      reject.click();
    ...
    >
      ...
      <Vision
        button={showVision() && <Button 
          ...
          />}
      />
      <button className="rejectbtn" style={{visibility: "hidden"}} data-option={selectedOption} data-type="Reject" data-cycle={cycle} onClick={handleUserReject}>Reject</button> 
      <Footer />
    </div>
  );
};
